...
class MultiTaskRegressor(nn.Module):
    def __init__(self):
        ...
        self.backbone.fc = nn.Linear(self.backbone.fc.in_features, 128)  # ResNet-18 kimeneti mérete 512

        # Kimeneti rétegek a két feladathoz
        self.fc_focus = nn.Linear(128, 1)  # Focus distance regression
        self.fc_class = nn.Linear(128, 4)  # Class classification

    def forward(self, x):
        features = torch.relu(self.backbone(x))  # ResNet-18-ből kinyert jellemzők
